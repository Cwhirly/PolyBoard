...
poly UCPF::Subset(poly __f, poly __g) {
    int __n = __f.size(), __lgn = __lg(__n);
    vector<poly> __F(__lgn + 1, poly(__n)), __G(__lgn + 1, poly(__n)),
        __H(__lgn + 1, poly(__n));
    for (int i = 0; i < __n; ++i) {
    ...
    for (int i = 0; i <= __lgn; ++i)
        for (int k = 0; k <= i; ++k) {
            int *__h = &__H[i][0], *__x = &__F[k][0], *__y = &__G[i - k][0];
            for (int S = 0; S < __n; ++S)
...
namespace CCNHLR {
    ...
    int Bostam_Mori_CCNHLR(int *c, int *a, int k, int n, const poly &__p0) {
        ...
        for (int i = 0; i < k; ++i)
            __p[i] = (a[i] + P - __p[i]) % P;
        __p.resize(k + __m + 1);
        vector<int> __x(__p.size() - 1);
        for (int i = 1; i <= __m + 1; ++i)
            __x[i - 1] = i + k - 1;
        poly __y = Multipoint_Evel::Multipoint().eval(__p0, __x);
        for (int i = 1; i <= __m + 1; ++i)
            __p[i + k - 1] = __y[i - 1];
        __g.resize(__m + k + 1);
        poly __A = __p * (~__g);
        ...
        __h.pb(1);
        __h *= __ksm(__m + 1);
        poly __z = __calc(__h, n);
        long long __ans = 0;
        for (int i = 0; i <= __m + k; ++i)
            __ans = (__ans + 1ll * __z[i] * __A[i]) % P;
        return __ans;
    }
}
