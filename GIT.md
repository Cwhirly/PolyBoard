...
### 现在的工作区布局（1.17.2 / 2.1.1 起）

工作区**只留两份代码**，旧版本不再摆在目录里（需要时从 tag 取）：

- `1_17_2.cpp`：不带矩阵的最新版
- `2_1_1.cpp`：带矩阵的最新版，**第一行就是 `#include "1_17_2.cpp"`**

```bash
git show 1.16.0:1_16_0.cpp > /tmp/1_16_0.cpp    # 把任意旧版本导出到别处看
```

2.x 能直接复用 1.x，是因为 `1_17_2.cpp` 末尾的 `work()` / `main()` 包在 `#if __INCLUDE_LEVEL__ == 0` 里：直接编译时才会带上，被 `#include` 时自动跳过，包含者可以写自己的 `work()` / `main()`。**以后新开 2.x 文件，第一行照抄 `#include "1_17_2.cpp"` 就行。**

> 这层守卫是在 2.1.1 的改造里才加的，所以 **`1.17.2` tag 里的那份文件没有它**（拿 tag 里的文件去 include 会报 `main` 重定义）。要用 include 复用，认准 **main 分支**上的 `1_17_2.cpp`；`git show 1.17.2:1_17_2.cpp` 取出来的那份只能单独编译。

另外每次更新顺手改两处文档：

- `CHANGELOG.md`：顶层一项对应一个版本，正文就是你写在发布页里的那段话
- `README.md`：接口有变化时同步（新接口写进对应章节，行为变化写进「相对 x.y.z 的变化」）
...
| 1.17.0 | `536f298` | 这个提交才把「子集反演」补进 `1_17_0.cpp`；同一个提交也把带矩阵的 `2_1_0.cpp` 加了进来（2.1.1 起工作区只留 `2_1_1.cpp`，`2_1_0.cpp` 用 `git show 536f298:2_1_0.cpp` 取） |
